...
// have to change things if you don't like openGL's default settings.
{
    ...
    GLfloat light_specular[] = { 1.0, 1.0, 1.0, 0.15 };
	GLfloat light_shininess = 100;
    //	light_position is NOT default value:
    GLfloat light_position[] = { 0.0, 2.0, 4.0, 0.0 };
...
//	glutWireSphere(0.1, 131, 131);
//	glColor3f(0.0,1.0,0.2);
//	glutWireSphere(0.1, 71, 71);
...
// ('special' keys such as return, function keys, arrow keys? keyboardspecial)
{
	switch(key) {
		...
		case 'n':					// shrink the object
			...
			break;
//		case 'a':
//			if (setModel.x_diff < 1.0f)
//			setModel.x_diff += 0.1f;
//			break;
//		case 'z':
//			if (setModel.x_diff > 0.0f)
//			setModel.x_diff -= 0.1f;
//			break;
		case 'r':
			setProj.reset();
